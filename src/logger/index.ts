...
function getTransports(config?: LoggerConfigurations): Transport[] {
     const fileEnabled = config?.logFileEnabled === 'true' ? true : false;
     const logstashEnabled = config?.logstashEnabled === 'true' ? true : false;
          ...
          format: winston.format.combine(
               ...
               winston.format.printf(
                    (info: winston.Logform.TransformableInfo) => {
                         ...
                         return `${info.timestamp} [${info.level}][service:${configsApp?.service}]${tags}: ${info.message}`;
                    },
               ),
          ),
